...
namespace CORE.JGC.Controllers
{
    ...
    public class UtilizationController : Controller
    {
        ...
        [HttpPost]
        public ActionResult SaveUpdateUser(string GroupMenuCode, string UserNameS, string Name, string Email, string Address, string Phone, string CompanyID, string DeptCode, string SiteCode, string LocationCode, int Floor)
        {
            try
            {
                ...
                try
                {
                    ...
                    var query = dc.UtilUser_IUD(UserNameS,"", Name, Email, Address, Phone, CompanyID, DeptCode, SiteCode, LocationCode, Floor, GroupMenuCode, 1, 0, UserID, 2);
                    string status = "";
                    foreach (var res in query)
                    {
                        status = res.Status;
